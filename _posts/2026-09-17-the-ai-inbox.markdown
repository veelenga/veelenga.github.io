...
Memo is my setup for personal notes in Notion, built on this pattern. The inbox is in Notion. The organizing is done by `memo-organize`, a [Claude skill](https://docs.claude.com/en/docs/agents-and-tools/agent-skills/overview) (a small instruction file an agent follows) that runs on a schedule.
